...
plot <- filter(metals, Element == "Cu" | Element == "P" | Element == "Sb" | Element == "Zn" | Element == "Sn") %>%
  ...
  geom_rect(aes(xmin = LS16BELL00$AgeLo[299], xmax = LS16BELL00$AgeHi[299], ymin = -Inf, ymax = Inf), alpha = 0.2, fill = "grey") +
...
```{r, echo=FALSE}
#plotOrg <- filter(organic, Element == "CN_mol" | Element == "C_V") %>%
plotOrg <- organic %>%
  drop_na() %>%
  ggplot(aes(AgeMed, Value)) +
  geom_line() +
